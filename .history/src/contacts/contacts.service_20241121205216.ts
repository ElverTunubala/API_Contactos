...
export class ContactService {
  constructor(
    @InjectRepository(ContactEntity,UserEntity)
  ...
  async create(createContactDto: CreateContactDto): Promise<ContactEntity> {
    try {
      ...
      const user = await this.userRepository.findOne({ where: { id: createContactDto.userId } });
      ...
      const contact = this.contactRepository.create({
        ...createContactDto,
        user, // Asignar el usuario encontrado a la relación user
      });
